...
int main(int argc, char const* argv[])
{
  ...
  const auto in = Linx::Image<T, 2>("input", in_extent, in_extent).fill_with_offsets_from_data();
  const auto k = Linx::Image<T, 2>("kernel", k_extent, k_extent).fill_with_offsets_from_data();
  print_2d(in);
  print_2d(k);
  Kokkos::fence();
