...
#if defined(_RASTER_TESTUTILS_IMPL) || defined(CHECK_QUALITY)

#include "Raster/TestUtils.h"

#include <algorithm> // generate
#include <chrono> // now (for seed)
#include <limits>
#include <random>
...
template <typename T>
std::vector<T> generateRandomVector(std::size_t size, T min, T max) {
...
generateRandomVector<std::complex<float>>(std::size_t size, std::complex<float> min, std::complex<float> max);

template <>
std::vector<std::complex<double>>
generateRandomVector<std::complex<double>>(std::size_t size, std::complex<double> min, std::complex<double> max);

template <>
std::vector<std::string> generateRandomVector<std::string>(std::size_t size, std::string min, std::string max);

template <typename T>
bool approx(T test, T ref, double /* tol */) {
  return test == ref;
}
