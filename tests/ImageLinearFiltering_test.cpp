...
BOOST_AUTO_TEST_CASE(correlation_impulse_response_test)
{
  ...
  in_on_host(0, 0) = 1;
  Kokkos::deep_copy(in.base(), in_on_host.base());
  ...
  Linx::for_each("test", k.domain(), KOKKOS_LAMBDA(int i, int j) { test(-i, -j) = Kokkos::conj(k(i, j)); });
  test -= out;
...
BOOST_AUTO_TEST_CASE(convolution_impulse_response_test)
{
  ...
  BOOST_TEST((out.domain() == test.domain()));
  Linx::for_each("test", k.domain(), KOKKOS_LAMBDA(int i, int j) { test(i + 2, j + 2) = k(i, j); });
  test -= out;
