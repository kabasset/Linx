...
struct StdSort {
  static const auto& nth(auto& in_out, Index n)
  {
    auto begin = &in_out[0];
    auto nth = begin + n;
 ...
 * @tparam TAlgo The sorting algorithm
 * 
...
auto median(TInOut& in_out)
{
  ...
  if constexpr (std::is_same_v<TParity, OddNumber>) {
    return TAlgo::sort_n(in_out, std::size(in_out) / 2);
  } else if constexpr (std::is_same_v<TParity, EvenNumber>) {
    const auto& high = TAlgo::sort_n(in_out, std::size(in_out) / 2 + 1);
    const auto& low = *(&high - 1);
    return std::midpoint(low, high);
  } else {
    if (size % 2 == 0) {
      return median<EvenNumber, TAlgo>(in_out);
    } else {
      return median<OddNumber, TAlgo>(in_out);
    }
  }
}
